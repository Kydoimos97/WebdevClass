...
if (isset($_POST['searchUser']) | array_key_exists('searchUser', $_POST)) {
    if (isset($_POST['searchName'])) {
        global $userData;
        $conn = mysqli_connect("localhost", "root", "", "urbanstore");
        $sql = "SELECT * FROM users where userName = '" . $_POST['searchName'] . "'";
        $result = mysqli_query($conn, $sql);
        if ($result->num_rows > 0) {
            global $userData;
            $userData = mysqli_fetch_array($result);
        } else {
            $_SESSION['message'] = "User email not found";
        }
    }
} elseif (isset($_SESSION['searchName']) & !empty($_SESSION['searchName'])) {
    $conn = mysqli_connect("localhost", "root", "", "urbanstore");
    $sql = "SELECT * FROM users where userName = '" . $_SESSION['searchName'] . "'";
    $result = mysqli_query($conn, $sql);
    $userData = mysqli_fetch_array($result);
} else {
    $conn = mysqli_connect("localhost", "root", "", "urbanstore");
...
if (isset($_POST['updateUser']) | array_key_exists('updateUser', $_POST)) {
    global $userData;
    require_once("SourceCode/Scripts/passwordCheckerSearch.php");
    if (isset($_SESSION['searchName']) && !empty($_SESSION['searchName'])) {
        accountCheck($userData);
    } else {
        $_SESSION['message'] = "No user defined";
    }
                        ...
                        <div class="card rounded-3 text-black">
                            ...
                            <form method="post" action>
                                <div class="row g-0">
                                    <div class=col-4></div>
                                    <div class=col-4>
                                        <div class="form-outline mb-4 w-15">
                                            <label class="form-label font-weight-bold" style="font-size: 10px"
                                                   for="searchName"> User Search:</label>
                                            <input list="names" name="searchName" id="searchName"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-8">
                                        <div class="text-center mx-auto pt-3 mb-2 pb-5 w-50">
                                            <button class="btn btn-primary btn-block default-button-main"
                                                    type="submit" name="searchUser" value="Submit">Search User
                                            </button>
                                        </div>
                                    </div>
                                    <div class="col-2"></div>
                            ...
                            <div class="row">
                                ...
                                <div class="col-8">
                                    <div class="card w-90 page-title border-0 center-block ">
                                        <p class="detail-title pt-0 pb-3 border-bottom"><?php if (!empty($_SESSION['searchName'])) {
                                                echo "Viewing Data for " . $_SESSION['searchName'];
                                            } else {
                                                echo "Customer data viewer";
                                            } ?></p>
                                    </div>
                                </div>
                                <div class="col-2"></div>
                                                ...
                                                <div class=row>
                                                    <div class=col>
                                                        ...
                                                        <select class="form-control"
                                                                ...
                                                                name="state">
                                                            <option value="<?php global $userData; if (isset($userData) && !empty($userData['state'])) echo $userData["state"]; else echo ""; ?>"><?php global $userData; if (isset($userData) && !empty($userData['state'])) echo $userData["state"]; else echo "N/A"; ?></option>
